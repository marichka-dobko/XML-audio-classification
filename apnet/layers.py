...
class PrototypeLayer(Layer):
    ...
    def __init__(self, n_prototypes, distance='euclidean', use_weighted_sum=True, use_attention=True, **kwargs):
        self.n_prototypes = n_prototypes
        self.distance = distance
        self.use_weighted_sum = use_weighted_sum
    ...
    def build(self, input_shape):
        ...
        if self.use_attention:
            self.attention_kernel = self.add_weight(name='prototype_attention_vectors',
                                                    shape=(input_shape[2],),
                                                    initializer='ones',
                                                    trainable=True)
        super(PrototypeLayer, self).build(input_shape)
    ...
    def call(self, x):
        ...
        if self.use_attention:
            x = K.mean(x, axis=(2, 3))
            x = K.expand_dims(x, axis=1)
            self.attention_kernel = K.expand_dims(self.attention_kernel, axis=0)
            attention_weights = K.softmax(K.dot(x, self.attention_kernel), axis=-1)
            attention_weights = K.expand_dims(attention_weights, axis=-1)
            self.kernel = self.kernel * attention_weights
...
class WeightedSum(Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(name='prototype_feature_vectors',
                                      shape=shape,
                                      initializer=Constant(value=1 / float(input_shape[2])),
